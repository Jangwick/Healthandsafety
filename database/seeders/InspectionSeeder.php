...
class InspectionSeeder
{
    public function run(PDO $db): void
    {
        ...
        $templates = $db->query("SELECT id FROM checklist_templates")->fetchAll(PDO::FETCH_COLUMN);
        
        if (empty($establishments) || empty($inspectors) || empty($templates)) return;
        ...
        $inspections = [
            ...
            [
                ...
                'inspector_id' => $inspectors[0],
                'template_id' => $templates[1] ?? $templates[0],
                'status' => 'Scheduled',
        ...
        foreach ($inspections as $insp) {
            $stmt = $db->prepare("INSERT INTO inspections (establishment_id, inspector_id, template_id, status, scheduled_date, score, rating) 
                                 VALUES (?, ?, ?, ?, ?, ?, ?)");
            $stmt->execute([
